    ...
    <div className="min-h-screen bg-gradient-to-br from-puchoo-warm-50 to-puchoo-warm-100">
      <header className="bg-white shadow-sm border-b border-puchoo-green-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-puchoo-green-dark">Gestão de Benefícios</h1>
              <p className="text-sm text-puchoo-terracotta">VT, VA, VR, Planos de Saúde, Consignados</p>
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          ...
          <Card>
            <CardContent className="pt-6 text-center">
              <p className="text-sm text-puchoo-terracotta">Custo Mensal Total</p>
          ...
          <CardContent>
            <div className="space-y-3">
              {(resumo.data as any)?.tipos?.map((t: any, i: number) => (
                <div key={i} className="flex items-center justify-between p-4 bg-puchoo-warm-50 rounded-lg hover:bg-puchoo-warm-100 transition-colors">
                  <div className="flex items-center gap-3">
                    {iconMap[t.tipo] || <Gift className="w-5 h-5 text-puchoo-terracotta" />}
                    <div>
                      <p className="font-medium">{t.tipo || "N/A"}</p>
                      <p className="text-sm text-puchoo-terracotta">{t.quantidade} colaboradores</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="font-semibold text-purple-700">{fmt(parseFloat(t.custoMensal?.toString() || "0"))}</p>
                    <p className="text-xs text-puchoo-terracotta">custo mensal</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <CardContent>
              <div className="space-y-3">
                {consignados.data?.emprestimos.map((e: any) => (
                  <div key={e.id} className="p-3 bg-puchoo-warm-50 rounded-lg">
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        <p className="font-medium text-sm">{e.colaboradorNome || "N/A"}</p>
                        <p className="text-xs text-puchoo-terracotta">{e.fornecedor || "N/A"}</p>
                      </div>
                      <Badge variant="outline">{e.status || "N/A"}</Badge>
                    </div>
                    <div className="grid grid-cols-3 gap-2 text-xs">
                      <div><span className="text-puchoo-terracotta">Total:</span> <span className="font-medium">{fmt(parseFloat(e.valorTotal?.toString() || "0"))}</span></div>
                      <div><span className="text-puchoo-terracotta">Parcela:</span> <span className="font-medium">{fmt(parseFloat(e.valorParcela?.toString() || "0"))}</span></div>
                      <div><span className="text-puchoo-terracotta">Restantes:</span> <span className="font-medium">{e.parcelasRestantes}/{e.parcelas}</span></div>
                    </div>
                  </div>
                ))}
                {consignados.data && (
